...
As with PageRank, substitute all $\mathbf{0}^T$ rows with $\frac{1}{n}\mathbf{1}^T$

![](./imgs/S.png)

-----

Now the fair-weather fan takes a long, random walk along this *Markov graph:*

![](./imgs/61-fair_wheather_random_walk.png)

-----

We record the number of times the random walker passes each vertex.

After a while, the proportion of visits to each node stabilizes.
...
If weights are all non-negative and sum to 1, also $\mathbf{S}$ will be stochastic.
...
$\mathbf{\overline{S}} = \beta \mathbf{S} + (1-\beta)[\frac{1}{n} \mathbf{1^T1}]$ (a matrix with $\frac{1}{n}$ everywhere)
...
Let's hire fairweather fans to do random walks:
...
- a m x n x 3 tensor where each layer, sometimes called *channel* contain [0..255] intensities of the respective color

. . .

a network of pixel nodes joint in a mesh: each node is connected rectilinearly with 2 (corner), 3 (border) or 4 (inner) neighbor pixels.


# Mapping

RGB values can be normalised to [0..1] by mapping the three values into *intensities,* i.e, the length of the vector over $N^3$
...
make the prob. to move to a neighbor pixel inverse-proportional to the difference in intensity between the origin and destination pixels.
