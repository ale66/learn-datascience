...
## From decomposition to data comprehension

Decompose the data matrix and interpret its 'top' eigenpairs as concepts/topics for user and activity classification:

$$M = U \Sigma V^T$$

. . .  

$U_{(m \times r)}$ is column-orthonormal: if $\mathbf{u_i}$ and $\mathbf{u_j}$ are columns of U, then $\mathbf{u_i}^T\cdot \mathbf{u_j}=0$

$V^T_{(r \times n)}$ is row-orthonormal: $\mathbf{v_i}^T\cdot \mathbf{v_j}=0$

$\Sigma_{(r \times r)}$ (or $D_{(r \times r)}$) is diagonal, with the $\sigma_{i}$s on the main diagonal.

dimension $r\leq \min\{m,n\}$ also depends on the no. of singular values found; reduce it further by dropping 0/very low $\sigma$s
...
Lee and Seung sought a new approach to the problem of learning images (how to recognise them) via visual patterns:

reduce a specific image to the combination of known patterns: face $\approx$ eyes, eyebrows, nose, chin etc.
...
Example dataset: faces from 'the wild' [[Papageorgiou-Poggio]](https://dl.acm.org/doi/abs/10.1023/A%3A1008162616689) (not Olivetti)
