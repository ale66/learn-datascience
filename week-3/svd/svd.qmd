...
$M_{(m \times m)} = Q\Lambda Q^{-1}$

. . . 

$M^{-1} = (Q\Lambda Q^{-1})^{-1}$

inverse factor order: $M^{-1} = (Q^{-1})^{-1}\Lambda^{-1}  Q^{-1}$

$M^{-1} = Q\Lambda^{-1} Q^{-1}$

where $\Lambda^{-1}$ is obtained by simply substituting each $\lambda_i$ on the diagonal with $\frac{1}{\lambda_i}$.
...
Notation: let $\vec{U_i}$ be the i-th column of matrix $U.$

The e-vectors that make up $U$ are always orthogonal with each other: $\vec{U_i}^T \cdot \vec{U_j}=0\ (i\neq j).$
...
over-constrained systems (more rows than cols.) can now be solved
...
Extract the eigenpairs: $\lambda_1 \approx 91$, $\vec{e}_1 = [0.15, 0.33, 0.52]$ 

Unfortunately, $\lambda_2 \approx \lambda_3 \approx 0$: orthogonal vectors are needed to make up our matrix U

(the third row is almost the sum of the first two, i.e., almost dependent)
...
- rows > columns: the problem is over-constrained (no solution?)

- rows < columns: the problem is over-parametrized (infinite sols.?)
...
If A is square (m=n) and non-singular ($|A|\neq 0$) then
...
Idea: strengthen up the main diagonal before inversion:
...
Thanks to $A^+$, over-constrained linear systems can now be solved (w. approximation): a new, general method for solving Phys/Eng. problems
...
where $D^+$, such that $D^+D=I$ is easy to calculate: if
